-- 045 — Harden face-match enforcement: when face_match_required is on, the user must
-- actually be enrolled (profiles.face_descriptor present), not merely report matched=true.
-- Same 7-arg signature as 043, so this replaces it cleanly (no new overload).
...
begin
  ...
  select is_field_staff, (face_descriptor is not null) into v_is_field, v_has_face
  ...
  -- Face-match enforcement: must be enrolled AND report a successful match.
  if coalesce(v_settings.face_match_required, false) then
    if not coalesce(v_has_face, false) then
      raise exception 'Enrol your face before punching.';
    end if;
    if coalesce(p_face_matched, false) is not true then
