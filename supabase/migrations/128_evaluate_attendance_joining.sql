-- 128 — evaluate_attendance: ignore days BEFORE the employee's date_of_joining.
-- Previously, past days with no punch were marked 'absent' (LOP=1) even for dates
-- before a person joined, inflating their LOP and under-paying mid-month joiners.
-- Now those days return status 'none' (not-applicable, no LOP) — the same shape as
-- a future day. Employees with no date_of_joining on file fall back to the old
-- behaviour (no regression). Only the pre-joining guard + the v_doj lookup are added;
-- all other evaluation logic is unchanged.
...
begin
  ...
  -- Employee's joining date (nullable → guard is skipped, old behaviour preserved).
  select date_of_joining into v_doj from public.employee_details where user_id = p_employee limit 1;
  ...
  while d <= p_to loop
    ...
    -- Before joining → not applicable (no LOP), same shape as a future 'none' day.
    if v_doj is not null and d < v_doj then
      work_date := d; status := 'none';
      first_in := null; last_out := null; worked_minutes := null;
      late_minutes := 0; early_minutes := 0; penalty := null;
      worked_units := 0; lop_units := 0; covered := null;
      return next;
      d := d + 1;
      continue;
    end if;
