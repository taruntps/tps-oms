-- 076: Fix punch_attendance blocking every face punch.
--
-- Root cause (found via postgres error log "Enrol your face before punching."):
-- punch_attendance gated on `profiles.face_descriptor IS NOT NULL` (the retired
-- on-device face engine). Enrollment moved to the face-refs storage bucket +
-- AWS Rekognition, so face_descriptor is NULL for everyone → the gate raised for
-- every enrolled user → verify-punch got a 500 → the punch never recorded and the
-- user saw "Enrol your face" despite being registered.
--
-- The attendance-verify-punch edge function already performs enrollment + face
-- matching server-side with an allow-and-flag model, so this function must NOT
-- re-gate on face. It also should not hard-block on geofence — punches are
-- recorded and FLAGGED (within_fence / is_field / face_matched / verification_status)
-- for admin review, never rejected. Accuracy + selfie checks remain as retry prompts.
...
CREATE OR REPLACE FUNCTION public.punch_attendance(
  ...
  p_selfie_path text DEFAULT NULL::text, p_device text DEFAULT NULL::text,
  p_face_matched boolean DEFAULT NULL::boolean, p_face_score numeric DEFAULT NULL::numeric)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
begin
  ...
  -- Face + geofence are recorded and flagged, never blocked (see header).
